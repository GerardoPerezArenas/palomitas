DECLARE @sge_generico varchar(30) = 'sge_generico';
DECLARE @idiomaDefecto varchar(30) = '1';

DECLARE @pais NUMBER(3,0) = 108;
DECLARE @municipio NUMBER(3,0) = 15;
...
ALTER VIEW [dbo].[V_CRO] ( CRO_OCU, PRO_COD,PRO_NOM,CRO_EJE,CRO_NUM,TRA_COD,TRA_NOM, CRO_FEI, CRO_FEF, USU_COD, USU_NOM, UTR_COD, 
							UTR_NOM, CRO_FIP, CRO_FLI, CRO_FFP, CRO_OBS, CRO_LENG, CRO_LOC, CRO_AYT, CRO_DAC, CRO_MAC, CRO_AAC, EXP_EST, 
							EXP_ASU, DNN_RCA, PRV_NOM, ESI_NOM, ECO_NOM, EXP_OBS ,COD_ORG)
AS
	SELECT E_CRO.CRO_OCU, E_CRO.CRO_PRO, E_PML.PML_VALOR, E_CRO.CRO_EJE, E_CRO.CRO_NUM, E_CRO.CRO_TRA, E_TML.TML_VALOR, 
			...
			T_ESI.ESI_NOM, T_ECO.ECO_NOM, E_EXP.EXP_OBS,E_CRO.CRO_MUN
	FROM E_CRO 
	...
	WHERE E_PML.PML_CMP='NOM'
		...
		AND T_MUN.MUN_PAI=@pais
		AND T_MUN.MUN_PRV=@municipio;
GO
		
		

if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[V_CROI]') and OBJECTPROPERTY(id, N'IsView') = 1)
drop view [dbo].[V_CROI]
GO
...
CREATE VIEW [V_CROI] (cro_ocu, pro_cod, pro_nom, cro_eje, cro_num, tra_cod, tra_nom, cro_fei, cro_fef, usu_cod, usu_nom, utr_cod, utr_nom,
							...
							eco_nom, dnn_cpo, EXP_OBS, CRO_DCE)
AS
	SELECT   e_cro.cro_ocu, e_cro.cro_pro, e_pml.pml_valor, e_cro.cro_eje, e_cro.cro_num, e_cro.cro_tra, e_tml.tml_valor,
			...
			(CASE
                   WHEN t_tvi2.tvi_des IS NOT NULL AND t_tvi2.tvi_des <> ' '
                      THEN t_tvi2.tvi_des
                   ELSE ''
                END
            )
            + ' ' +
			(CASE
                   WHEN t_via2.via_nom IS NOT NULL AND t_via2.via_nom <> ' '
                      THEN t_via2.via_nom
                   ELSE ''
                END
            )
           ...
           + (CASE
            ...
            t_dnn2.dnn_rca, t_prv.prv_nom, t_esi.esi_nom, t_eco.eco_nom, t_dnn.dnn_cpo, E_EXP.EXP_OBS, HTE_DCE
	...
	WHERE   e_pml.pml_cmp = 'NOM'
		AND e_tml.tml_cmp = 'NOM'
		AND e_pml.pml_leng = e_tml.tml_leng
		AND mun2.mun_pai = @pais
		AND mun2.mun_prv = @municipio;
GO		
...
-- #104883 Errores: La vista V_EXP_INF no devuelve resultados en algunas instalaciones
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[V_EXP_INF]') and OBJECTPROPERTY(id, N'IsView') = 1)
drop view [dbo].[V_EXP_INF]
GO
CREATE VIEW [V_EXP_INF] 
(
  EXP_LOC, 
  EXP_FEI,
  EXP_FEF, 
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[V_INT]'))
DROP VIEW [dbo].[V_INT]
GO

CREATE VIEW [V_INT] (INT_DCE,INT_MUN, INT_EJE, INT_NUM, INT_TER, INT_NVR, INT_CDT, INT_NOR, INT_NOC, INT_NOM, INT_AP1, INT_AP2, INT_TID,
							INT_DOC, INT_DOMN, INT_DOMNN, INT_ROL, INT_RPD, INT_POB, INT_TLF, INT_RCA, INT_PRV, INT_ECO, INT_ESI, INT_CPO,INT_CODEXTERNO)
AS
	SELECT t_hte.hte_dce,e_ext.ext_mun,e_ext.ext_eje,e_ext.ext_num,e_ext.ext_ter,e_ext.ext_nvr,e_ext.ext_dot, t_dom.dom_nml,
			CAST(hte_nom AS VARCHAR)
				+ ' '
				+ CAST(t_hte.hte_pa1 AS VARCHAR)
				+ ' '
				...
				+ ''
				+ CAST(t_hte.hte_ap2 AS VARCHAR) AS titular,
			 t_hte.hte_doc,hte_nom,hte_ap1,hte_ap2,t_tid.tid_des,
	...
	FROM  e_ext
		...
		LEFT OUTER JOIN  t_eco  ON
			...
			AND	t_esi.esi_prv  = t_eco.eco_prv
			AND	t_esi.esi_eco  = t_eco.eco_cod ,
		 t_dom, t_dpo, t_via, t_tvi ,t_tid,t_ter
	...
	UNION
	SELECT t_hte.hte_dce,e_ext.ext_mun,e_ext.ext_eje,e_ext.ext_num,e_ext.ext_ter,e_ext.ext_nvr,e_ext.ext_dot,t_dom.dom_nml, hte_noc AS titular,
			hte_nom,hte_ap1,hte_ap2,t_tid.tid_des,hte_doc,NULL AS domicilio,
			(CASE
				WHEN t_tvi.tvi_des IS NOT NULL AND t_tvi.tvi_des <> ' '
					THEN t_tvi.tvi_des + ' '
					ELSE '' END)
	...
	FROM e_ext
		...
		LEFT OUTER JOIN t_eco ON
			...
			AND t_esi.esi_mun = t_eco.eco_mun
			AND t_esi.esi_prv = t_eco.eco_prv
			AND t_esi.esi_eco = t_eco.eco_cod), t_ter
		WHERE t_ter.ter_cod = e_ext.ext_ter;
GO
...
"REG_TRP_NOM", "REG_USU","REG_NUM","EXR_NUM","UOR_REG_COD_VIS","UOR_NOM_REG","CODIGO_ASUNTO","DESCRIPCION_ASUNTO","RES_TER") 
AS
SELECT ISNULL(CONVERT(char,A.RES_FEC,103),'')  AS RES_FEC,
        ISNULL(CONVERT(char,A.RES_FED,103),'')  AS RES_FED,
        A.RES_ASU,
        UOR_COD_VIS, 
		A.RES_UOD, 
		A.RES_UOR, 
	    ...
	    r_ttr.TTR_DES, 
		A_USU.USU_NOM, 
		CONVERT(VARCHAR,A.RES_EJE) + '/' +  CONVERT(VARCHAR,A.RES_NUM),
		E_EXR.EXR_NUM,
...
FROM R_RES A, R_TDO, R_TPE, R_TTR, A_UOR, T_HTE, @sge_generico.dbo.A_USU A_USU,E_EXR,R_TIPOASUNTO
WHERE 	A.RES_TDO *= R_TDO.TDO_IDE AND
      	A.RES_TPE *= R_TPE.TPE_IDE AND
      	A.RES_TTR *= R_TTR.TTR_IDE AND
      	A.RES_TER *= HTE_TER AND
		A.RES_TNV *= HTE_NVR AND
      	A.RES_UOD = UOR_COD AND
		A_USU.USU_COD = A.RES_USU
...
AS
SELECT DISTINCT CONVERT(VARCHAR,   res_fec,   121) AS res_fec,  
CONVERT(VARCHAR,   res_fed,   121) AS res_fed, res_asu, uor_cod_vis,
  		...
  		r_tpe.tpe_cod, r_tpe.tpe_des, r_ttr.ttr_cod AS cod_transp, r_ttr.ttr_des, a_usu.usu_nom, CONVERT(VARCHAR,   r_res.res_eje) + '/' +
  		CONVERT(VARCHAR,   r_res.res_num) AS num, hte_doc,
  		(CASE
...
(exr_dep         =res_dep
 ...
 CONVERT(VARCHAR,   res_fed,   121) AS res_fed, res_asu, uor_cod_vis,
  		res_uod,  res_uor, uor_nom, res_obs, hte_nom + isnull(' ',   '') + isnull(hte_pa1,   '') + isnull(' ',   '') + isnull(hte_ap1,   '') +
  		isnull(' ',   '') + isnull(hte_pa2,   '') + isnull(' ',   '') + isnull(hte_ap2,   '') AS interesado, res_tip, r_tdo.tdo_cod, r_tdo.tdo_des,
  		r_tpe.tpe_cod, r_tpe.tpe_des, r_ttr.ttr_cod AS cod_transp, r_ttr.ttr_des, a_usu.usu_nom, 
		CONVERT(VARCHAR,   r_res.res_eje) + '/' +
  		CONVERT(VARCHAR,   r_res.res_num) AS num, hte_doc,
  		(CASE
			WHEN res_mod = 0 THEN 'ORDINARIA'
			WHEN res_mod = 1 THEN 'DESTINO OTRO REGISTRO'
...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[V_REG_INICIO_EXP]') and OBJECTPROPERTY(id, N'IsView') = 1)
drop view [dbo].[V_REG_INICIO_EXP]
GO
CREATE VIEW [V_REG_INICIO_EXP] ("NUMERO_REGISTRO", "FECHA_PRESENTACION", "NUM_EXPEDIENTE")
AS
  SELECT 
	(CONVERT(varchar,res_eje) + '/' + CONVERT(varchar,res_num))AS NUMERO_REGISTRO,
	CONVERT (varchar, res_fec, 13)		 AS FECHA_PRESENTACION,
