...
function Tabla200(paginar,buscar,anterior,siguiente,mosFilasPag,msgNoResultBusq,mosPagDePags,noRegDisp,filtrDeTotal,primero,ultimo,par,anchoTabla){
  ...
  this.displayTabla = TB_displayTabla200;
...
function TB_displayTabla200(){
    ...
    for(var i=0; i < this.lineas.length; i++){
      ...
      for(var j=0; j < lin.length; j++){	  
        if(this.columnas[j][0]>0) {
            ...
            str += '<td style="align:' + this.columnas[j][2]+';width:'+anchoCol + ';" onclick="selectRow(' + i + ',' + 
   ...
   try{
    ...
    this.dataTable = $("#"+this.id).DataTable( {
            ...
            "lengthMenu": [ 10, 25, 50, 100, 200, 300, 350, 400, 450, 500 ],
            "autoWidth": false,
            "aaSorting": [],
            "language": {
                "search": this.buscar,
