...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
					...
					<td width="70mm" class="fila" align="left">
						<table width="60mm" cellpadding="0mm" border="0">
							<xsl:for-each select="./TRAMITE">		   
                                ...
                                <tr>
                                    <td width="60mm">
                                        (Fecha inicio: <xsl:value-of select="FECHA_INICIO"/>)
                                    </td>
                                </tr>
                                <xsl:if test="(//CRITERIOS/TIPO_LISTADO = 1)">
						            <tr>
                                        <td width="60mm">
                                            (Fecha fin: <xsl:value-of select="FECHA_FIN"/>)
                                        </td>
                                    </tr>
					            </xsl:if>
                            </xsl:for-each>
	...
	<xsl:template name = "etiquetaTramites" > TRAMITES PENDIENTES</xsl:template>
    <xsl:template name = "etiquetaTramitesFinalizados" > TRAMITES FINALIZADOS</xsl:template>
    <xsl:template name = "etiquetaLocalizacion" > LOCALIZACION</xsl:template>
	<xsl:template name = "etiquetaClasificacionTramite" > CLASIFICACION DE TRAMITE</xsl:template>
