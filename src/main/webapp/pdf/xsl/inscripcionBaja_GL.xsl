		...
		<xsl:template name="pagina">
			<xsl:param name="nh">1</xsl:param>
				...
				<tr>
					<td height="60mm">
						<xsl:choose>
						<xsl:when test="titulo='BAIXA POR CAMBIO DE RESIDENCIA' and datosHabitante/habitante[position() = ($nh)]/codProvinciaDestino!= '' and datosHabitante/habitante[position() = ($nh)]/codMunicipioDestino!= ''">
