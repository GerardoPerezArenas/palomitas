...
<%
    ...
    try {
        ...
        if (hostVirtual.equals("")) {
            urlHost = protocolo + "://" + request.getHeader("host") + request.getContextPath() + "/portafirmas/documentoportafirmas/SearchDocumentoPortafirmas.do";
        } else {
            urlHost = hostVirtual + request.getContextPath() + "/portafirmas/documentoportafirmas/SearchDocumentoPortafirmas.do";
        }
    } catch (Exception e) {
        e.printStackTrace();
    }
...
function btnCloseRechazoOtroDocumento_click(theSender,theForm) {
    if ( (theSender) && (theForm) ) {
    theForm.name   = "SearchDocumentoPortafirmasForm";
    theForm.action = "<%=urlHost%>";
    theForm.idNumeroDocumento.disabled = true;
...
function btnCloseRechazoDocumentoProcedimiento_click(theSender,theForm) {
    if ( (theSender) && (theForm) ) {
    theForm.name   = "SearchDocumentoPortafirmasForm";
    theForm.action = "<%=urlHost%>";
    theForm.idNumeroDocumento.disabled = true;
    theForm.idNumFirma.disabled = true;
    theForm.idPresentado.disabled = true;
    pleaseWait1("on",this);
    theForm.submit();
    }
} 

function btnCloseSubsanaDocumentoProcedimiento_click(theSender,theForm) {
    if ( (theSender) && (theForm) ) {
        theForm.name   = "SearchDocumentoPortafirmasForm";
        theForm.action = "<%=urlHost%>";
        theForm.idNumeroDocumento.disabled = true;
