...
<body class="bandaBody" scroll=no onload="javascript:{pleaseWait('off');}">
        ...
        <tr>
            <td width="791px" height="415px">
                <table width="100%" height="100%" cellpadding="0px" cellspacing="1px" border="0px" bgcolor="#ffffff">
                    <tr>
                        <td id="titulo" class="txttitblanco"><%= descriptor.getDescripcion("gEtiq_MantInformes")%></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td>
                            <table width="100%" height="100%" align="center" cellpadding="0px" cellspacing="3px" border="0px" bgcolor="#e6e6e6">
                                ...
                                <td align="center" class="etiqueta">Aplicaci&oacute;n:&nbsp;&nbsp;&nbsp;&nbsp;<html:text style="visibility:hidden;height:0;width:0;" styleId="codAplicacion" property="codAplicacion"/> <% System.err.println("Llego5");%><input class="inputTexto" type="text" id="descAplicacion" name="descAplicacion" style="width:200;">&nbsp;<a href="" id="anchorAplicacion" name="anchorAplicacion"><span class="fa fa-chevron-circle-down" aria-hidden="true"  id="botonAplicacion" name="botonAplicacion" style="cursor:hand;"></span></a></td>
                                </tr>
                                <tr><td>&nbsp;</td>
                                </tr>
                                <tr>
    ...
    <script type="text/javascript">
      ...
      function Inicio()
      {
        ...
        ncomboCampoOrdenacion_1.change=eventoCambiaAplicacion;
        EventoCambioTabla();
            }
    ...
    var tab = new Tabla(true,'<%=descriptor.getDescripcion("buscar")%>','<%=descriptor.getDescripcion("anterior")%>','<%=descriptor.getDescripcion("siguiente")%>','<%=descriptor.getDescripcion("mosFilasPag")%>','<%=descriptor.getDescripcion("msgNoResultBusq")%>','<%=descriptor.getDescripcion("mosPagDePags")%>', '<%=descriptor.getDescripcion("noRegDisp")%>','<%=descriptor.getDescripcion("filtrDeTotal")%>','<%=descriptor.getDescripcion("primero")%>','<%=descriptor.getDescripcion("ultimo")%>',document.getElementById('tabla'));
        tab.addColumna('45','center','<%=descriptor.getDescripcion("gEtiq_CodInforme")%>');
        tab.addColumna('170','left','<%=descriptor.getDescripcion("gEtiq_NombreInforme")%>');
        tab.addColumna('430','left','<%=descriptor.getDescripcion("gEtiq_DescInforme")%>');
    ...
    function callbackCubreTablaInformes(vInformes)
    {
        ...
        CubreControles();
            ...
            tab.clearTabla();
            jsp_alerta('A','<%=descriptor.getDescripcion("gEtiq_NoInf")%>');
        }
        EventoCambioTabla();
    ...
    } else jsp_alerta('A','<%=descriptor.getDescripcion("msjElegirInf")%>');
    }
    ...
    {
    ejecutaDocumentoDesdeInformes(urlDOC,unescape(XML),false);
    }
    ...
    function ModificarInforme(cod){
      ...
      abrirXanelaAuxiliar('<%=request.getContextPath()%>/jsp/informes/mainVentana.jsp?source='+source,null,
