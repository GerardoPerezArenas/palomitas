...
<script type="text/javascript" src="<%=request.getContextPath()%>/scripts/utilidadesXerador.js"></script>

<meta http-equiv="" content="text/html; charset=iso-8859-1">
</head>
<body scroll=no onload="javascript:{pleaseWait('off');}">
...
<div class="contenidoPantalla">
    ...
    <div id="tabla2" class="botoneraPrincipal">
        <input type= "button" class="botonGeneral" value="<%= descriptor.getDescripcion("gbVisualizar")%>" name="cmdVisualizar" onClick="PulsaBoton(event);"> 
        ...
        <input type= "button" class="botonGeneral" value="<%= descriptor.getDescripcion("Etiq_EditarPlantilla")%>" name="cmdEditarPlantilla" onClick="PulsaBoton(event);">
...
<script type="text/javascript">
  ...
  function pulsarSalir(){
    ...
    document.forms[0].submit();//window.location = "/padron/Gestion.do?opcion=inicializarListasCriterios&operacion=CONSULTA_PADRON";
  }
...
//
var URLBaseAction='<%=request.getContextPath()%>/XeradorInformesAction.do?operacion=';
...
if (tab.selectedIndex>-1) {
	...
	if (lin[3]=='S')
		...
		jsp_alerta('A','Debe editar primero la plantilla para el informe con código:'+COD_INFORMEXERADOROculto.value+'.');
} else
...
function ModificarInforme(cod) {
    abrirXanelaAuxiliar("<%=request.getContextPath()%>/jsp/informes/main2.jsp?op=M&id="+cod+"&codApliInforme="+codAplicacion.value,'ventana',
	'width=795,height=580,status='+ '<%=statusBar%>',function(datos){
                            if(datos!=undefined){
                                    recargarLista();
                            }
                    });
...
function AltaInforme(codAplicacion) {
    abrirXanelaAuxiliar("<%=request.getContextPath()%>/jsp/informes/main2.jsp?op=A&id="+codAplicacion,null,
...
function EditarPlantillaInforme(cod){
	...
	document.forms[0].action="<%=request.getContextPath()%>/jsp/informes/main2.jsp?op=GEP&id="+cod;
