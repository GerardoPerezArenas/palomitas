...
<% /* Recuperar el vector de usuarios de la sesion. */
   UsuariosFormsForm f= (UsuariosFormsForm)  session.getAttribute("UsuariosFormsForm");
   UsuariosFormsValueObject ugVO = f.getUsuariosForms();
...
%>
   parent.mainFrame.listaOriginal[<%= j %>]  = ['<%= cU %>','<%= l %>','<%= n %>'];
   parent.mainFrame.lista[<%= j++ %>]  = ['<%= l %>','<%= n %>'];
