 ...
 <head><jsp:include page="/jsp/sge/tpls/app-constants.jsp" />
    ...
    function pulsarAceptar(){ 
      ...
      for(i=0;i<filas.length;i++){
        ...
        //listaCamposOriginal[indice][7] = valor;
        listaCamposOriginal[indice][4] = valor;
      }
      for(i=0;i<listaCamposOriginal.length;i++) {
        j=0
        while ((!encontrado) && (j<listaCamposOriginal.length)) {
            if (i == listaCamposOriginal[j][4] - 1) {
      ...
      if(campoID!=null && campoID!=''){
        ...
        if(campo) {
          if(campoID==activo){
            ...
            campo.className="ordenCampoActivo";
            campoActivo = campoID;
          }
        }
      }
 ...
 <body onload="javascript:{inicializar();}" >
    ...
    <div class="contenidoPantalla">
        <table width="100%">
            <tr>
              <td width="90%">
                <table width="100%" bordercolor="#7B9EC0" border="1" cellspacing="0" cellpadding="0" class="fondoCab" align="center">
                  <tr height="20">
                    <td width="10%" bgcolor="#7B9EC0" class="cabecera" align="center"><%= descriptor.getDescripcion("etiq_orden")%></td>
                    <td width="20%" bgcolor="#7B9EC0"  class="cabecera" ><%= descriptor.getDescripcion("gEtiq_codigo")%></td>
                    <td width="70%" bgcolor="#7B9EC0" class="cabecera" ><%= descriptor.getDescripcion("gEtiq_desc")%></td>
                  </tr>
                </table>
              </td>
              <td width="10%">
          ...
          <td width="90%">
            <div style="height:350px;overflow:auto;">
                  <table cellpadding="2" id="tblCampos" width="100%">
                  </table>
        <script type="text/javascript">
                ...
                for(var i=0;i<listaCamposOriginal.length;i++) {                                
                  ...
                  //filap.id=listaCamposOriginal[i][7];
                  filap.id=listaCamposOriginal[i][4]; // Orden
                  filap.onclick=selectCampo;
                  var celdap=filap.insertCell(0);
                  var dh = '<table width="100%"><tr>';
                    //dh += '<td width="10%" class="etiqueta">'+listaCamposOriginal[i][7]
                    dh += '<td width="10%" class="etiqueta">'+listaCamposOriginal[i][4]
                    +'</td><td width="20%" class="etiqueta">';
                  dh += listaCamposOriginal[i][0]
                  + '</td><td width="70%" class="etiqueta" >'+listaCamposOriginal[i][1]
                  + '</td></tr></table>';
                  //celdap.innerHTML='<div class="ordenCampo" id="div-'+listaCamposOriginal[i][7]+'" >'+ dh +'</div>';
                  celdap.innerHTML='<div class="ordenCampo" id="div-'+listaCamposOriginal[i][4]+'" >'+ dh +'</div>';
                }
        </script>
        </div>
      </td>
      <td width="10%">
            <table border="0" cellpadding="10" cellspacing="0">
                <tr>
                  <td valign="top">
                    <input name="asc" type="image" onclick="subeCampo();return false;" src="<c:url value='/images/flecha_up.gif'/>">
                </td>
              </tr>
              <tr>
                <td valign="bottom">
                    <input name="desc" type="image" onclick="bajaCampo();return false;" src="<c:url value='/images/flecha_down.gif'/>">
                </td>
              </tr>
            </table>
        </td>
