...
<% /* Recuperar el vector de asientos de la sesion para el buzon. */
   ...
   int numRelacionAsientos = relacionAsientos.size();

   int j=0;
   for (int i=0; i<relacionAsientos.size(); i++ ) {
      ...
      a = a.replaceAll("\"", "");
      String obs = tram.getObservaciones();
      if(obs != null){  
          obs = obs.replaceAll("\"", "");
      } else {
          obs = "";
...
%>
    var valorCheck = <%=numAnotacion[0]%>+DOT_COMMA+<%=numAnotacion[1]%>+DOT_COMMA+"E"+DOT_COMMA+<%=uR%>+DOT_COMMA+"<%=tecnicoReferencia%>";
    var asunto = unescape("<%= a %>");
    var observaciones = unescape("<%= obs %>").replaceAll("<", "&lt;").replaceAll(">", "&gt;");
    parent.mainFrame.listaOriginal[<%= j %>] = ["<%= d %>","<%= uR %>","<%= tR %>","<%= eN %>","<%= fA %>",
            "<%= r %>",asunto, "<%=origenServicio%>", "<%=codAsuntoCodif%>", "<%=descAsuntoCodif%>", "<%=tecnicoReferencia%>", observaciones];
        
        if (asunto.length > 55 ) asunto = asunto.substring(0,55) + "...";
        if (observaciones.length > 55 ) observaciones = observaciones.substring(0,55) + "...";
   ...
   parent.mainFrame.lista[<%= j %>]  = [check,"Pendiente","<%=procedimiento%>","<%=numExp%>","<%=usu%>","<%= eN %>","<%=tipo%>","<%= fA %>",
               "<%= fechaDoc %>","<%= r %>",asunto,observaciones,"<%= uor %>","Ordinaria","<%=tipoTransporte%>","<%=uorInicio%>", "<%=codAsuntoCodif%>", "<%=descAsuntoCodif%>", "<%=tecnicoReferencia%>"];
