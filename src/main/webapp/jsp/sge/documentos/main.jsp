...
<html:html>
 ...
 <head>
    ...
    <%
      ...
      DocumentosExpedienteForm formulario=null;
      ...
      if (session!=null)
      {
        ...
        formulario=(DocumentosExpedienteForm)session.getAttribute("DocumentosExpedienteForm");
        if (formulario!=null){ 
            numeroExpediente=formulario.getNumeroExpediente();
        }
      }
...
<body class="bandaBody" onload="Inicio()">
    ...
    If (Not IsNull(document.forms(0).numeroDocumento.Value) And Not IsEmpty(document.forms(0).numeroDocumento.Value) And Not(document.forms(0).numeroDocumento.Value="")) Then
        plantilla = "<%=dominioFinal%>/temp/expedientes/<%=params[0]%>/<%=params[6]%>/"& _
                    document.forms(0).codMunicipio.value & "/" &_
                    replace(document.forms(0).numeroExpediente.value,"/","+") & "/" &_
        ...
        Call verDocumento(plantilla,False, "<%= dominioFinal %>")
    Else
        ...
        End If
        Call ejecutaDocumento(plantilla,document.forms(0).datosXML.Value,False,"<%= dominioFinal%>")
    End If
    End Sub
    </script>
 </body>
