...
function ModificarInforme(cod){
  ...
  lin=tab.getLinea(tab.selectedIndex);
  COD_INFORMEXERADOROculto.value=lin['CODIGO'];
  var source = "<%=request.getContextPath()%>/informes/GeneradorInformes.do?opcion=recuperarInforme&operacion="+codAplicacion.value+"&tipoFichero="+COD_INFORMEXERADOROculto.value;
  abrirXanelaAuxiliar("<%=request.getContextPath()%>/jsp/registro/mainVentana.jsp?source="+source,null,
...
function AltaInforme(codAplicacion){
  var source = "<%=request.getContextPath()%>/informes/GeneradorInformes.do?opcion=inicio&operacion="+codAplicacion;
  abrirXanelaAuxiliar("<%=request.getContextPath()%>/jsp/registro/mainVentana.jsp?source="+source,null,
        'width=750,height=475,status='+ '<%=statusBar%>',function(datos){
            if(datos!=undefined){
                recargarLista();
...
function EditarPlantillaInforme(cod){
	...
	document.forms[0].target="oculto";
		document.forms[0].action="<%=request.getContextPath()%>/jsp/informes/main2.jsp?op=GEP&id="+cod+"&nombrePlantilla="+lin[1];
	document.forms[0].submit();
}
}
