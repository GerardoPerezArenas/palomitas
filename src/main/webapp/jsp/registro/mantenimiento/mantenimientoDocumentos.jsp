<!-- JSP de Mantenimiento de Documentos -->
    ...
    <link rel="stylesheet" type="text/css" href="<c:url value='/css/font-awesome-4.6.2/less/animated.less'/>" media="screen" >
        ...
        <script type="text/javascript">
            ...
            function pulsarEliminar() {
                ...
                if(tab.selectedIndex != -1){
                    if(validarFormulario()){
                        ...
                        document.forms[0].action = '<%=request.getContextPath()%>/MantDocumento.do';
            ...
            function pulsarRecuperar(){
                ...
                if(tab.selectedIndex != -1){
                    if(validarFormulario()){
                        if(lista[tab.selectedIndex][3] == "BAJA"){
                            var id = lista_ide[tab.selectedIndex];
                            document.forms[0].identificador.value = id;
                            document.forms[0].opcion.value="recuperar";
                            document.forms[0].target = "oculto";
                            document.forms[0].action='<%=request.getContextPath()%>/MantDocumento.do';
                            document.forms[0].submit();
                            limpiarInputs();
                    ...
                    if(tab.selectedIndex != -1) {
                        if (validarFormulario()) {
                            ...
                            for(z=0; z < lista.length; z++){
                                if ((lista[z][2] == 'SI' )&&(document.forms[0].activo.checked==true)){
                                    modificar = 1;
                                    ide=lista_ide[z][0];
                                }
                            }
        ...
        function pulsarAlta() {
            ...
            if (validarFormulario()) {
                if(cod!=0){
                    ...
                    jsp_alerta('A','<%=descriptor.getDescripcion("msjCodigoNoValido")%>');
                }
            }
        }

function pulsarSalir(){
    window.location = "<%=request.getContextPath()%>/MantDocumento.do?opcion=salir";
}
...
function checkKeysLocal(evento,tecla) {
    ...
    if (teclaAuxiliar == 13) buscar(teclaAuxiliar);
    keyDel(evento);
}
...
<form method="post">
    ...
    <div id="titulo" class="txttitblanco"><%=descriptor.getDescripcion("titMantDoc")%></div>
    <div class="contenidoPantalla" valign="top">
            ...
            <tr>
                <td>
                    ...
                    <input type="text" class="inputTextoObligatorio" id="obligatorio"  
                           name="codigo"  maxlength="2" onblur="return xAMayusculas(this);" 
                           style="width:11%">
                    <!-- Descripcion -->
                    <input type="text" class="inputTextoObligatorio" id="obligatorio" 
                           name="txtNomeDescripcion" maxlength="70" 
                           onblur="return xAMayusculas(this);" style="width:56.5%">
                    <!-- Activo -->
        ...
        <div class="botoneraPrincipal">
            <input type= "button" class="botonGeneral" value=<%=descriptor.getDescripcion("gbAlta")%> name="cmdAlta" onClick="pulsarAlta();" alt="<%= descriptor.getDescripcion("toolTip_bAltaTDoc")%>" title="<%= descriptor.getDescripcion("toolTip_bAltaTDoc")%>">
            ...
            <input type= "button" class="botonGeneral" value=<%=descriptor.getDescripcion("gbRecuperar")%> name="cmdRecuperar" onClick="pulsarRecuperar();" title='<%=descriptor.getDescripcion("toolTip_bRecuperar")%>' alt='<%=descriptor.getDescripcion("toolTip_bRecuperar")%>'>
...
<script>
            ...
            tab.addColumna('94','center','<%=descriptor.getDescripcion("gEtiq_estado")%>');
            tab.addColumna('94','center','<%=descriptor.getDescripcion("gEtiq_fecEstado")%>');
                    ...
                    function cargaTabla(){
                        ...
                        lista_ide[cont] = ['<bean:write name="elemento" property="ide"/>'];
                            lista[cont] = ['<bean:write name="elemento" property="codigo" />', '<bean:write name="elemento" property="txtNomeDescripcion"/>',
