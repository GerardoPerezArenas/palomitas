...
<html>
    <head><jsp:include page="/jsp/administracion/tpls/app-constants.jsp" />
        ...
        <title>Mantenimiento de Tipos de Departamentos</title>
    ...
    <%for (i = 0; i < lengthDepartamentos; i++) {
                GeneralValueObject departamentos = (GeneralValueObject) listaDepartamentos.get(i);%>
                    datosDepartamentos[j] = ['<%=(String) departamentos.getAtributo("codigo")%>',
                        ...
                        j++;
                        <%}%>
                        tablaDepartamentos.lineas = lista;
                        refresca(tablaDepartamentos);
                    }
                    
                    // FUNCIONES DE LIMPIEZA DE CAMPOS
                    function limpiarFormulario(){
                        tablaDepartamentos.lineas = new Array();
                        refresca(tablaDepartamentos);
                        limpiarInputs();
                    }
                    ...
                    function limpiar() {
                        limpiarInputs();
                        tablaDepartamentos.selectLinea(-1);
                    }
                    ...
                    function pulsarEliminar() {
                        var vector = [document.forms[0].Codigo];
                        habilitarGeneral(vector);
                        if(tablaDepartamentos.selectedIndex != -1) {
                            document.forms[0].identificador.value = lista[tablaDepartamentos.selectedIndex][0];
                            document.forms[0].opcion.value = 'eliminar';
                            document.forms[0].target = "oculto";
                            document.forms[0].action = '<%=request.getContextPath()%>/administracion/mantenimiento/Departamentos.do';
                            document.forms[0].submit();
                        }
                            ...
                            if(tablaDepartamentos.selectedIndex != -1){
                                if(validarFormulario()){
                                    for(l=0; l < lista.length; l++){
                                        ...
                                        lineaSeleccionada = tablaDepartamentos.selectedIndex;
                                    ...
                                    function selecFila(des){
                                        if(des.length != 0){
                                            for (var x=0; x<lista.length; x++){
                                                ...
                                                if(auxLis == des){
                                                    if(x!=tablaDepartamentos.selectedIndex) tablaDepartamentos.selectLinea(x);
                                                    return;
                                                }
                                            }
                                        }
...
<form name="formulario" method="post">
    ...
    <div id="titulo" class="txttitblanco"><%= descriptor.getDescripcion("gEtiq_MantDepartamen")%></div>
            ...
            <tr> 
                <td>
                    ...
                    <input name="Descripcion" type="text" class="inputTextoObligatorio" id="obligatorio" 
                           maxlength="90" onblur="return xAMayusculas(this);"  style="width:86%"
                           onKeyUp="buscar(tablaDepartamentos);">
                </td>
            </tr>
        </table>
    ...
    <script type="text/javascript">
            var tablaDepartamentos = new Tabla(true,'<%=descriptor.getDescripcion("buscar")%>','<%=descriptor.getDescripcion("anterior")%>','<%=descriptor.getDescripcion("siguiente")%>','<%=descriptor.getDescripcion("mosFilasPag")%>','<%=descriptor.getDescripcion("msgNoResultBusq")%>','<%=descriptor.getDescripcion("mosPagDePags")%>', '<%=descriptor.getDescripcion("noRegDisp")%>','<%=descriptor.getDescripcion("filtrDeTotal")%>','<%=descriptor.getDescripcion("primero")%>','<%=descriptor.getDescripcion("ultimo")%>',document.getElementById('tabla'));
            
            tablaDepartamentos.addColumna('120','center','<%= descriptor.getDescripcion("gEtiq_Codigo")%>');
                tablaDepartamentos.addColumna('780',null,'<%= descriptor.getDescripcion("gEtiq_Descrip")%>');
                    tablaDepartamentos.displayCabecera = true;
                    ...
                    if(window.event){
                        ...
                        keyDel(evento);
                        if (teclaAuxiliar == 38) upDownTable(tablaDepartamentos,lista,teclaAuxiliar);
                        if (teclaAuxiliar == 40) upDownTable(tablaDepartamentos,lista,teclaAuxiliar);
                        if (teclaAuxiliar == 13) pushEnterTable(tablaDepartamentos,lista);
                    }
