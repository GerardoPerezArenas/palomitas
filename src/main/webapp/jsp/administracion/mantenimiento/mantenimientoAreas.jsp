...
<html>
    <head><jsp:include page="/jsp/administracion/tpls/app-constants.jsp" />
        ...
        <title>Mantenimiento de Tipos de Areas</title>
                ...
                GeneralValueObject areas = (GeneralValueObject) listaAreas.get(i);%>
                    datosAreas[j] = ['<%=(String) areas.getAtributo("codigo")%>',
                        ...
                        listaOriginal[j] = ['<%=(String) areas.getAtributo("codigo")%>',
                            ...
                            j++;
                            <%}%>
                            tablaAreas.lineas = lista;
                            refresca(tablaAreas);
                        }
                        
                        // FUNCIONES DE LIMPIEZA DE CAMPOS
                        function limpiarFormulario(){
                            tablaAreas.lineas = new Array();
                            refresca(tablaAreas);
                            limpiarInputs();
                        }
                        ...
                        function pulsarEliminar() {
                            if(tablaAreas.selectedIndex != -1) {
                                if(jsp_alerta("C",'<%=descriptor.getDescripcion("desEliminarAre")%>') ==1) {
                                    var vector = [document.forms[0].Codigo];
                                    habilitarGeneral(vector);
                                    document.forms[0].identificador.value = lista[tablaAreas.selectedIndex][0];
                                    ...
                                    document.forms[0].action = '<%=request.getContextPath()%>/administracion/mantenimiento/Areas.do';
                                    document.forms[0].submit();
                                }
                            }
                            ...
                            function noEliminar() {
                                jsp_alerta('A', '<%=descriptor.getDescripcion("msjNoElimArea")%>');
                                    limpiar();
                                }
                                
                                function pulsarModificar(){
                                    var cod = document.forms[0].Codigo.value;
                                    var codCamp = document.forms[0].codCampo.value;
                                    var codId = document.forms[0].codIdioma.value;
                                    var yaExiste = 0;
                                    if(tablaAreas.selectedIndex != -1){
                                        if(validarFormulario()){
                                            for(l=0; l < lista.length; l++){
                                                var lineaSeleccionada;
                                                lineaSeleccionada = tablaAreas.selectedIndex;
...
<form name="formulario" method="post">
    ...
    <div id="titulo" class="txttitblanco"><%= descriptor.getDescripcion("gEtiq_MantAreas")%></div>
        ...
        <tr> 
            <td> 
                <input type="text" class="inputTextoObligatorio" id="obligatorio"
                       name="Codigo" maxlength="3" style="width:13.5%" 
                      onkeyup="return SoloDigitosNumericos(this);">
                <input name="Descripcion" type="text" class="inputTextoObligatorio" id="obligatorio" 
                       maxlength="80" style="width:85.5%" onblur="return xAMayusculas(this);"
                       onKeyUp="buscar(tablaAreas);">
            </td>
        </tr>
    </table>								
...
<script type="text/javascript">
            var tablaAreas = new Tabla(true,'<%=descriptor.getDescripcion("buscar")%>','<%=descriptor.getDescripcion("anterior")%>','<%=descriptor.getDescripcion("siguiente")%>','<%=descriptor.getDescripcion("mosFilasPag")%>','<%=descriptor.getDescripcion("msgNoResultBusq")%>','<%=descriptor.getDescripcion("mosPagDePags")%>', '<%=descriptor.getDescripcion("noRegDisp")%>','<%=descriptor.getDescripcion("filtrDeTotal")%>','<%=descriptor.getDescripcion("primero")%>','<%=descriptor.getDescripcion("ultimo")%>',document.getElementById('tabla'));
            
            tablaAreas.addColumna('120','center','<%= descriptor.getDescripcion("gEtiq_Codigo")%>');
                tablaAreas.addColumna('780','left','<%= descriptor.getDescripcion("gEtiq_Descrip")%>');
                    tablaAreas.displayCabecera=true;
                    tablaAreas.displayTabla();
                    ...
                    function checkKeysLocal(evento, tecla){
                       var teclaAuxiliar = "";
                        ...
                        if (teclaAuxiliar == 38) upDownTable(tablaAreas,lista,teclaAuxiliar);
                        if (teclaAuxiliar == 40) upDownTable(tablaAreas,lista,teclaAuxiliar);
                        if (teclaAuxiliar == 13) pushEnterTable(tablaAreas,lista);
                    }
