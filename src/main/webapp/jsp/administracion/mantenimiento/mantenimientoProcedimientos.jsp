...
<html>
    <head><jsp:include page="/jsp/administracion/tpls/app-constants.jsp" />
        ...
        <title>Mantenimiento de Tipos de Procedimientos</title>
                ...
                GeneralValueObject procedimientos = (GeneralValueObject) listaProcedimientos.get(i);%>
                    datosProcedimientos[j] = ['<%=(String) procedimientos.getAtributo("codigo")%>',
                        ...
                        listaOriginal[j] = ['<%=(String) procedimientos.getAtributo("codigo")%>',
                            ...
                            j++;
                            <%}%>
                            tablaProcedimientos.lineas = lista;
                            refresca(tablaProcedimientos);
                        }
                        
                        // FUNCIONES DE LIMPIEZA DE CAMPOS
                        function limpiarFormulario(){
                            tablaProcedimientos.lineas = new Array();
                            refresca(tablaProcedimientos);
                            limpiarInputs();
                        }
                        ...
                        function limpiar() {
                            limpiarInputs();
                            tablaProcedimientos.selectLinea(-1);
                        }
                        ...
                        function pulsarEliminar() {
                            if(tablaProcedimientos.selectedIndex != -1) {
                                if(jsp_alerta("C",'<%=descriptor.getDescripcion("desEliminarTipProc")%>') ==1) {
                                    var vector = [document.forms[0].Codigo];
                                    habilitarGeneral(vector);		
                                    document.forms[0].identificador.value = lista[tablaProcedimientos.selectedIndex][0];;
                                    ...
                                    document.forms[0].action = '<%=request.getContextPath()%>/administracion/mantenimiento/Procedimientos.do';
                                    document.forms[0].submit();
                                }
                            }
                                ...
                                if(tablaProcedimientos.selectedIndex != -1){
                                    if(validarFormulario()){
                                        for(l=0; l < lista.length; l++){
                                            ...
                                            lineaSeleccionada = tablaProcedimientos.selectedIndex;
                                    ...
                                    function selecFila(des){
                                        if(des.length != 0){
                                            for (var x=0; x<lista.length; x++){
                                                ...
                                                if(auxLis == des){
                                                    if(x != tablaProcedimientos.selectedIndex)
                                                        tablaProcedimientos.selectLinea(x);
                                                    break;
                                                }
                                            }
                                        }
    ...
    <body class="bandaBody" onload="javascript:{pleaseWait('off');
        ...
        <form name="formulario" method="post">
            ...
            <div id="titulo" class="txttitblanco"><%= descriptor.getDescripcion("gEtiq_MantProced")%></div>
                    ...
                    <tr>
                        <td>
                            ...
                            <input name="Descripcion" type="text" class="inputTextoObligatorio" id="obligatorio" 
                                   ...
                                   onblur="return xAMayusculas(this);" onKeyUp="buscar(tablaProcedimientos);">
                        </td>
                    </tr>
                </TABLE>
...
-        </form>
        <script type="text/javascript">
            var tablaProcedimientos = new Tabla(true,'<%=descriptor.getDescripcion("buscar")%>','<%=descriptor.getDescripcion("anterior")%>','<%=descriptor.getDescripcion("siguiente")%>','<%=descriptor.getDescripcion("mosFilasPag")%>','<%=descriptor.getDescripcion("msgNoResultBusq")%>','<%=descriptor.getDescripcion("mosPagDePags")%>', '<%=descriptor.getDescripcion("noRegDisp")%>','<%=descriptor.getDescripcion("filtrDeTotal")%>','<%=descriptor.getDescripcion("primero")%>','<%=descriptor.getDescripcion("ultimo")%>',document.getElementById('tabla'));
            
            tablaProcedimientos.addColumna('120','center','<%= descriptor.getDescripcion("gEtiq_Codigo")%>');
                tablaProcedimientos.addColumna('780','left','<%= descriptor.getDescripcion("gEtiq_Descrip")%>');
                    tablaProcedimientos.displayCabecera=true;
                    tablaProcedimientos.displayTabla();
                    ...
                    function checkKeysLocal(evento, tecla){
                        ...
                        if (teclaAuxiliar == 38) upDownTable(tablaProcedimientos,lista,teclaAuxiliar);
                        if (teclaAuxiliar == 40) upDownTable(tablaProcedimientos,lista,teclaAuxiliar);
                        if (teclaAuxiliar == 13) pushEnterTable(tablaProcedimientos,lista);
                    }
