...
<title> Listado de Instrumentos de Planeamiento </title>
<%
...
function pulsarImprimir() {
		...
		document.forms[0].action="<html:rewrite page='/planeamiento/ImprimirInstrumentoPlaneamiento.do'/>?idioma=" + "<%=idioma%>";
...
function pulsarSalir(){
    window.location = "<html:rewrite page='/planeamiento/CargarInstrumentoPlaneamiento.do'/>?tipoRegistro=1&consultando=true";
}
...
function rellenarDatos(tableObject, rowID){
    if(rowID>-1 && !tableObject.ultimoTable){
        ...
        document.forms[0].codigoTipo.value = lista[rowID][4];
    }
}

function callFromTableTo(rowID,tableName){
    document.forms[0].target="mainFrame";
    document.forms[0].action = "<html:rewrite page='/planeamiento/RellenarInstrumentoPlaneamientoFromListado.do'/>";
    document.forms[0].submit();
}
...
<table width="100%" height="100%" cellpadding="0px" cellspacing="1px" border="0px" bgcolor="#ffffff">
    <tr>
        <td id="titulo" class="txttitblanco"><%= descriptor.getDescripcion("tit_listInsPlan")%></td>
    </tr>
    <tr>
...
<script type="text/javascript">
    ...
    tab.addColumna('0','left','');
    tab.addColumna('190','left','<%= descriptor.getDescripcion("gEtiq_tipo")%>');
    ...
    function cargaTabla(){
        ...
        lista = new Array();
    <logic:iterate id="elemento" name="registrosPlaneamiento" scope="session">
        lista[cont] = ['<bean:write name="elemento" property="anho"/>/<bean:write name="elemento" property="numero"/>', '<bean:write name="elemento" property="fechaAprobacion"/>', '<bean:write name="elemento" property="codigoSubseccion"/>', '<bean:write name="elemento" property="subseccion"/>', '<bean:write name="elemento" property="codigoTipo"/>', '<bean:write name="elemento" property="tipo"/>', '<bean:write name="elemento" property="ambito"/>'];
        cont = cont + 1;
    </logic:iterate>
