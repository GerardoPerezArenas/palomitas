...
function limpiarFormulario(){
    tablaPromotores.lineas = new Array();
    tablaPromotores.displayTabla();
...
function pulsarEliminar() {
    if(tablaPromotores.selectedIndex != -1) {
        if (validarFormulario()) {
            document.forms[0].target = "oculto";
            document.forms[0].action = "<html:rewrite page='/planeamiento/EliminarPromotor.do'/>";
...
function pulsarAlta() {
    ...
    if(validarFormulario() && document.forms[0].descripcion.value!="") {
        if(noEsta()){
            document.forms[0].target = "oculto";
            document.forms[0].action = "<html:rewrite page='/planeamiento/InsertarPromotor.do'/>";
            document.forms[0].submit();
            limpiarFormulario();
        } else {
...
function noEsta(){
    ...
    for(i=0;(i<listaPromotores.length);i++){
        if((listaPromotores[i][0] == cod))
            return false;
    }
    return true;
}
...
<table width="90%" cellpadding="0px" cellspacing="0px" align="center" style="margin-top: 5px;">
    <tr>
        <td width="791px" height="315px">
            <table id="tabla1" width="100%" height="100%" cellpadding="0px" cellspacing="1px" border="0px" bgcolor="#ffffff">
                <tr>
                    <td class="txttitblanco">&nbsp;Lista de Promotores
                    </td>
                </tr>
                <tr>
...
<script type="text/javascript">
    ...
    else tablaPromotores = new Tabla(true,'<%=descriptor.getDescripcion("buscar")%>','<%=descriptor.getDescripcion("anterior")%>','<%=descriptor.getDescripcion("siguiente")%>','<%=descriptor.getDescripcion("mosFilasPag")%>','<%=descriptor.getDescripcion("msgNoResultBusq")%>','<%=descriptor.getDescripcion("mosPagDePags")%>', '<%=descriptor.getDescripcion("noRegDisp")%>','<%=descriptor.getDescripcion("filtrDeTotal")%>','<%=descriptor.getDescripcion("primero")%>','<%=descriptor.getDescripcion("ultimo")%>',document.getElementById('tabla'));

    tablaPromotores.addColumna('0','center','');
    tablaPromotores.addColumna('500','center','<%=descriptor.getDescripcion("gEtiqNombre")%>');
    tablaPromotores.displayCabecera=true;
    tablaPromotores.height = 150;

    function cargaTabla(){
        var cont = 0;

        listaPromotores = new Array();
        <logic:iterate id="elemento" name="InstrumentoPlaneamientoForm" property="codigosPromotor">
            listaPromotores[cont] = ['<bean:write name="elemento" property="codigo" />', '<bean:write name="elemento" property="descripcion"/>'];
            cont = cont + 1;
        </logic:iterate>

        tablaPromotores.lineas=listaPromotores;
        refresh();
    }

    function rellenarDatos(tableObject, rowID){
        if(rowID>-1 && !tableObject.ultimoTable){
            document.forms[0].codigo.value = listaPromotores[rowID][0];
            document.forms[0].descripcion.value = listaPromotores[rowID][1];
        }else limpiarFormulario();
    }

    function recuperaDatos(lista) {
        listaPromotores = lista;
        tablaPromotores.lineas=listaPromotores;
        refresh();
    }
    ...
    function checkKeysLocal(evento,tecla){
        ...
        keyDel(evento);
        if (teclaAuxiliar == 38 ||teclaAuxiliar == 40) upDownTable(tablaPromotores,listaPromotores,teclaAuxiliar);
    }
</script></body></html>
