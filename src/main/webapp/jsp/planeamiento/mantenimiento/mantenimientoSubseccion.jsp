...
<%@ page import="es.altia.agora.interfaces.user.web.planeamiento.form.mantenimiento.MantenimientoSubseccionesForm"%>

<html>
<head><jsp:include page="/jsp/planeamiento/tpls/app-constants.jsp" />
<title>Mantenimiento de Subsecciones</title>
    ...
    if(tab.selectedIndex != -1) {
        if (validarFormulario()) {
            ...
            if(existe == 1) {
                ...
                document.forms[0].action = "<html:rewrite page='/planeamiento/mantenimiento/ModificarSubseccion.do'/>";
...
function pulsarAlta() {
    ...
    if (validarFormulario()) {
        ...
        if(yaExiste == 0) {
            document.forms[0].target = "oculto";
            document.forms[0].action = "<html:rewrite page='/planeamiento/mantenimiento/InsertarSubseccion.do'/>";
            document.forms[0].submit();
        } else {
...
function pulsarEliminar() {
    if(tab.selectedIndex != -1) {
        if (validarFormulario()) {
            document.forms[0].target = "oculto";
            document.forms[0].action = "<html:rewrite page='/planeamiento/mantenimiento/EliminarSubseccion.do'/>";
            document.forms[0].submit();
            limpiarInputs();
        }
    } else {
...
function pulsarSalir(){
    window.location = "<html:rewrite page='/planeamiento/mantenimiento/SalirSubseccion.do'/>";
}
    ...
    <tr>
        <td width="791px" height="415px">
            <table width="100%" height="100%" cellpadding="0px" cellspacing="1px" border="0px" bgcolor="#ffffff">
                <tr>
                    <%
                        String titulo = "tit_mantSubsec" + ((MantenimientoSubseccionesForm) session.getAttribute("MantenimientoSubseccionesForm")).getTipoRegistro();
                    %>
                    <td id="titulo" class="txttitblanco"><%= descriptor.getDescripcion(titulo)%></td>
                </tr>
                <tr>
                                    ...
                                    <table bgColor="#FFFFFF" width="700px" heigth="75%" align="center">
                                        <tr>
                                            <td width="10%" align="left">
                                                <html:hidden name="MantenimientoSubseccionesForm" property="tipoRegistro"/>
                                            </td>
                                            <!-- Codigo -->
                                            <td width="10%" align="left">
...
<script type="text/javascript">
    ...
    function cargaTabla(){
        ...
        lista = new Array();
    <logic:iterate id="elemento" name="MantenimientoSubseccionesForm" property="subsecciones">
