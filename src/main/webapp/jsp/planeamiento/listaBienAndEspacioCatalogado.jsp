...
<title> Listado de Bienes y Espacios Catalogados </title>
<%
...
function pulsarImprimir() {
		...
		document.forms[0].action="<html:rewrite page='/planeamiento/ImprimirBienAndEspacioCatalogado.do'/>?idioma=" + "<%=idioma%>";
...
function pulsarSalir(){
    window.location = "<html:rewrite page='/planeamiento/CargarBienAndEspacioCatalogado.do'/>?tipoRegistro=3&consultando=true";
}
...
function callFromTableTo(rowID,tableName){
    ...
    document.forms[0].action = "<html:rewrite page='/planeamiento/RellenarBienAndEspacioCatalogadoFromListado.do'/>";
    document.forms[0].submit();
}
...
<table width="100%" height="100%" cellpadding="0px" cellspacing="1px" border="0px" bgcolor="#ffffff">
    <tr>
        <td id="titulo" class="txttitblanco"><%= descriptor.getDescripcion("tit_listBien")%></td>
    </tr>
    <tr>
...
<script type="text/javascript">
    ...
    tab.addColumna('410','left','<%= descriptor.getDescripcion("gEtiq_denomBien")%>');
    tab.addColumna('190','left','<%= descriptor.getDescripcion("gEtiq_catalog")%>');
    tab.height = 300;
    ...
    function cargaTabla(){
        ...
        lista = new Array();
        var denominacionBien;
        var busca = /@intro@/gi;
        var pStrReemplaza = new String(String.fromCharCode(13)+String.fromCharCode(10));
    <logic:iterate id="elemento" name="registrosBien" scope="session">
        denominacionBien = '<bean:write name="elemento" property="denominacionBien"/>';
        denominacionBien = denominacionBien.replace(busca, pStrReemplaza);
        lista[cont] = ['<bean:write name="elemento" property="anho"/>/<bean:write name="elemento" property="numero"/>', '<bean:write name="elemento" property="fechaAprobacion"/>', denominacionBien, '<bean:write name="elemento" property="catalogacion"/>'];
        cont = cont + 1;
    </logic:iterate>
